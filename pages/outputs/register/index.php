...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['botaoCadastar'])) {
  ...
  try {
    ...
    // Selecionando output para pegar a quantidade já existente
    $output = "SELECT * FROM tb_output WHERE id_clifor = :id_clifor AND id_user = :id_user AND id_product = :id_product";
    $stmt = $conn->prepare($output);
    ...
    if (isset($result[0]) == 1) {
      ...
      $sql = "UPDATE tb_output SET amount = :amount WHERE id = :idoutput";
      $stmt = $conn->prepare($sql);
      $stmt->bindParam(':amount',  $somaValores);
      $stmt->bindParam(':idoutput', $result[0]['id']);
      $stmt->execute();

      $_SESSION['msg'] = 'Entrada atualizada';
      header("Location: ../index.php");
    } else {
      $sql = "INSERT INTO tb_output (id_clifor, id_user, id_product, amount, date) VALUES (:id_clifor, :id_user, :id_product, :amount, :date)";
